	...
	public String salvarCambios() throws Exception {
		...
		try {
			...
			if (!listaCasillasVacia) {
				...
				FacesMessage facesMsg = new FacesMessage(FacesMessage.SEVERITY_INFO,
						"Se ha agregado un nuevo formulario con �xito", "");
				FacesContext.getCurrentInstance().addMessage(null, facesMsg);
	...
	public String actualizarFormulario() throws Exception {
		...
		try {
			...
			if (!listaCasillasVacia) {
				...
				FacesMessage facesMsg = new FacesMessage(FacesMessage.SEVERITY_INFO, "Formulario actualizado con �xito",
						null);
				FacesContext.getCurrentInstance().addMessage(null, facesMsg);
				// Guardar el mensaje en el flash scope
				FacesContext.getCurrentInstance().getExternalContext().getFlash().setKeepMessages(true);
	...
	public void eliminarFormulario(String idFormulario) throws Exception {
		...
		try {
			if (filtroFormularios == null) { //Si no estamos filtrando
				...
				FacesMessage facesMsg = new FacesMessage(FacesMessage.SEVERITY_INFO, "Formulario eliminado con �xito",
						"");
				FacesContext.getCurrentInstance().addMessage(null, facesMsg);
